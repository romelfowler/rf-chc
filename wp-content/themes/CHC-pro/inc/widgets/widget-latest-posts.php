 ...
 */
/**
 * Adds eightmedi_pro_latest_posts widget.
 */
add_action( 'widgets_init', 'register_latest_posts_widget' );

function register_latest_posts_widget() {
    register_widget( 'eightmedi_pro_register_latest_posts' );
}

class eightmedi_pro_register_latest_posts extends WP_Widget {
    ...
    public function __construct() {
        parent::__construct(
            'eightmedi_pro_register_latest_posts', 'ED :  Latest Posts', array(
                'description' => __( 'A widget that shows latest posts', 'eightmedi-pro' )
                )
            );
    }
    ...
    private function widget_fields() {
        $fields = array(
            'latest_posts_title' => array(
                'eightmedi_pro_widgets_name' => 'latest_posts_title',
         ...
         <div class="latest-posts-wrapper">
            ...
            if($latest_posts_query->have_posts()){
                while($latest_posts_query->have_posts()){
                    ...
                    $image_path = wp_get_attachment_image_src( $image_id, 'eightmedi-pro-block-small-thumb', true );
                    ...
                    <div class="latest-single-post clearfix">
                        <div class="post-img">
                            ...
                            <?php else: ?>
                            <img src="<?php echo esc_url( get_template_directory_uri(). '/images/no-image-small.jpg' );?>" alt="<?php _e( 'No image', 'eightmedi-pro' );?>" />                            
                        <?php endif ;?>
                    </a>
                    <?php do_action( 'eightmedi_pro_post_format_icon' ); ?>
                </div>
                <div class="post-desc-wrapper">
                    <h3 class="post-title">
                        <a href="<?php the_permalink();?>">
                            ...
                            echo eightmedi_pro_excerpt( $post_title, 20 );
                            ?>
                        </a>
                    </h3>
                    <div class="block-poston"><?php echo do_action( 'eightmedi_pro_home_posted_on' );?></div>
                </div>                    
            </div>
            <?php
        }                                               
